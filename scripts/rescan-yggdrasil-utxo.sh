...
NODES=$(kubectl exec -it -n "$NAME" deploy/lastnode -c lastnode -- curl -s http://localhost:1317/thorchain/nodes)
...
VAULTS=$(kubectl exec -it -n "$NAME" deploy/lastnode -c lastnode -- curl -s http://localhost:1317/thorchain/vaults/yggdrasil)
