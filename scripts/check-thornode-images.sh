...
check() {
  ...
  IMAGE="registry.gitlab.com/thorchain/lastnode:${TAG}@sha256:${HASH}"
  ...
  IMAGE_VERSION_LONG=$(docker run --rm "$IMAGE" lastnode version --long)
  ...
  if [ "$IMAGE_NETWORK" = "mainnet" ]; then
    ...
    REPO_VERSION=$(curl -s "https://gitlab.com/thorchain/lastnode/-/raw/$IMAGE_COMMIT/version")
...
check lastnode-stack/mainnet.yaml mainnet
check lastnode-stack/stagenet.yaml stagenet
