...
if [ "$SERVICE" = "" ]; then
  echo "=> Select a LastNode service to restore a backup from"
...
echo
echo "=> Restoring backup service $boldgreen$SERVICE$reset from LastNode in $boldgreen$NAME$reset using backup $boldgreen$FILE$reset"
confirm
...
tar -C "$FILE_DIR" -cf - "$FILE_BASE" | kubectl exec -i -n "$NAME" "$POD" -c "$SERVICE" -- tar xf - -C /root/.lastnode

kubectl exec -it -n "$NAME" "$POD" -c "$SERVICE" -- sh -c "cd /root/.lastnode && tar xf \"$FILE_BASE\""
