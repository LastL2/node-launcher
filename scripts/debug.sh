...
IMAGE=$(kubectl -n "$NAME" get deploy/thor-daemon -o jsonpath='{$.spec.template.spec.containers[:1].image}')
SPEC="
{
  \"apiVersion\": \"v1\",
  \"spec\": {
    ...
    \"volumes\": [{\"name\": \"data\", \"persistentVolumeClaim\": {\"claimName\": \"thor-daemon\"}}]
  }
}"
